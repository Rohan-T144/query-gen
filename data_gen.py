...
https://github.com/ml-explore/mlx-examples/blob/main/lora/utils.py
"""
...
class WikiSQL:
    def __init__(self, dataset, save_dir="/tmp"):
        ...
        data_dir = os.path.join(save_dir, "wikisql_data")
        ...
        with open(tables) as f:
            for line in f:
                ...
                self._tables[table["id"]] = {
                    ...
                    "types": table["types"],
                    "desc": f"table: {table_id2name(table['id'])}\ncolumns: {', '.join(table['header'])}",
                }
    ...
    def query_to_text(self, query, table, columns, types):
        ...
        sql = f"SELECT {aggregation}{column} FROM {table_id2name(table)}"
